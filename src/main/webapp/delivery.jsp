...
<body>
    ...
    <form method="POST" action='?action=save' name="frmAddDelivery">
      ...
      <input type="text" name="senders_first_name"  value="<c:out value="${delivery.sendersFirstName}" />" /> <br />
       ...
       <label for="fromCity"> <fmt:message key="delivery.label.fromCity"  bundle="${rb}"/>:</label> <select name="city_from_id">
      ...
      <label for="goodsTypeId"> <fmt:message key="delivery.label.goodsTypeId"  bundle="${rb}"/>:</label>  <select name="goods_type_id">
                     <c:forEach items="${types}" var="type">
                      <option value="${type.idGoodsType}" ${type.idGoodsType==delivery.goodsType ? 'selected' : ''} >${type.name}</option>
                      </c:forEach>
                      </select><br />

        <label for="weight"> <fmt:message key="delivery.label.weight"  bundle="${rb}"/>:</label><input type="text" name="weight"
                       value="<c:out value="${delivery.weight}" />" /> <br />
      <label for="sendersPhone"> <fmt:message key="delivery.label.sendersPhone"  bundle="${rb}"/>:</label><input type="text" name="senders_phone"
                    value="<c:out value="${delivery.sendersPhone}" />" /> <br />
        <label for="recipientPhone"> <fmt:message key="delivery.label.recipientPhone"  bundle="${rb}"/>:</label> <input type="text" name="recipient_phone"
                   value="<c:out value="${delivery.recipientPhone}" />" /> <br />

        <label for="sentDate"> <fmt:message key="delivery.label.sentDate"  bundle="${rb}"/>:</label> <input   type="text" name="sent_date"
                 value="<fmt:formatDate pattern="yyyy-MM-dd" value="${delivery.sentDate}" />" /> <br />

       <label for="deliveryDate"> <fmt:message key="delivery.label.deliveryDate"  bundle="${rb}"/>:</label> <input    type="text" name="delivery_date"
